...
# from DeepCDR.prog.layers.graph import GraphLayer,GraphConv
...
#         x = Dropout(0.1)(x)
#         x = Lambda(lambda x: K.expand_dims(x,axis=-1), output_shape=lambda s: (s[0], s[1], 1))(x)
#         x = Lambda(lambda x: K.expand_dims(x,axis=1), output_shape=lambda s: (s[0], 1, s[1], s[2]))(x)
...
from keras.optimizers import Adam
from keras.regularizers import l2
from DeepCDR.prog.layers.graph import GraphConv, GraphLayer
